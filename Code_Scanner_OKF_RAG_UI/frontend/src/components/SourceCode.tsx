 ...
 */
export function SourceCode({ source, maxHeight = 480 }: { source: SourceView; maxHeight?: number }) {
    ...
    <div className="space-y-1">
      ...
      <pre data-testid="source-code" style={{ maxHeight }}
        ...
        {lines.map((text, i) => {
          ...
          const cls = commentLines.has(n) ? "text-emerald-700 dark:text-emerald-400"
            : conditionLines.has(n) ? "bg-amber-100/70 dark:bg-amber-900/30" : "";
          return (
            <div key={n} className={`flex px-2 ${cls}`}>
