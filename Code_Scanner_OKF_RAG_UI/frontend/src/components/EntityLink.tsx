...
export function EntityLink({ id, label, type, status, showType = false, className = "" }: Props) {
  ...
  const external = status === "external" || type === "external";
  return (
    <button
      type="button"
      onClick={() => open(id)}
      title={unresolved ? `${id} (unresolved reference)` : external ? `${id} (external: JDK/library)` : id}
      ...
      } ${external ? "!text-zinc-500 dark:!text-zinc-400" : ""} ${className}`}
    >
